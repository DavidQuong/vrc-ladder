package ca.sfu.cmpt373.alpha.vrcladder.notifications.methods;


public class NotificationSettings {
    public static final String ADDED_TO_TEAM              = "teamcreated";
    public static final String TEAM_TIME_UPDATED          = "teamtimeupdated";
    public static final String GAME_SCHEDULED             = "gamescheduled";
    public static final String GAME_SCHEDULED_REMINDER    = "gamescheduledreminder";
    public static final String GAME_SCORES_ENTERED        = "gamescorereported";
    public static final String ACCOUNT_ACTIVATED          = "registrationconfirmed";
    public static final String PASSWORD_UPDATED           = "passwordupdated";
    public static final String PASSWORD_RESET             = "passwordreset";
    public static final String FAILED_LOGIN               = "loginfailed";
    public static final String GAME_SCORES_REPORTED_WRONG = "gamescoresreportedwrong";
    public static final String GAME_SCORES_UPDATED        = "gamescoresupdated";


}
